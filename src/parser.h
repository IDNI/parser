...
public:
	...
	bool print_to_rules = false;
	bool print_to_dot = false;
	bool print_to_tml = false;
...
private:
	...
	ostream& put(ostream& os, const size_t& n) const {
		for (const auto& ch : to_string_(n))
			os.put((CharT) ch);
		return os;
	}
...
private:
	...
	struct overlay_tree {
		...
		nidx_t &root;
		parser::parse_forest_t &pf;
		overlay_tree(nidx_t &_r, parser::parse_forest_t &_pf):
		root(_r), pf(_pf){};
	};
};

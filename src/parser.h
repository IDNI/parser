...
struct lit {
	...
	size_t hashit() const {
		...
		if (std::holds_alternative<size_t>(data)) {
			seed ^= std::hash<size_t>{}(std::get<size_t>(data)) + 0x9e3779b9 + (seed << 6) + (seed >> 2);
		} else {
			seed ^= std::hash<T>{}(std::get<T>(data)) + 0x9e3779b9 + (seed << 6) + (seed >> 2);
		}
		return seed;
	}
...
public:
	...
	private:
		// avoid recursion
		std::set<typename parser<C, T>::pnode> done;

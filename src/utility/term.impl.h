...
#ifndef _WIN32
inline struct termios orig_attrs, raw_attrs;
#else
inline DWORD orig_in_mode = 0,  raw_in_mode = 0;
inline DWORD orig_out_mode = 0, raw_out_mode = 0;
inline HANDLE hIn = INVALID_HANDLE_VALUE;
inline HANDLE hOut = INVALID_HANDLE_VALUE;
inline bool init() {
...
inline void enable_getline_mode() {
...
inline void disable_getline_mode() {
	if (!opened) return;
#ifndef _WIN32
	tcsetattr(STDIN_FILENO, TCSANOW, &raw_attrs);
#else
...
inline void clear() { [[maybe_unused]] int result = std::system(
#ifndef _WIN32
	"clear"
#else
	"cls"
...
#ifdef _WIN32
inline std::deque<char> input_buffer;
inline std::vector<char> translate_virtual_key(WORD vk, DWORD controlState) {
...
inline void out(const char* data, size_t size) {

...
ostream_t& operator<<(ostream_t& os,
	const vector<typename parser<CharT>::string>& v)
{
...
template <typename CharT>
ostream_t& operator<<(ostream_t& os,
	const vector<typename parser<CharT>::lit>& v)
{
	int i = 0;
...
template <>
parser<char>::string parser<char>::to_str(const string_t &s) const {
	return to_string(s);	
}

template <>
parser<char32_t>::string parser<char32_t>::to_str(const string_t &s) const {
	return to_u32string(s);
}

template <typename CharT>
parser<CharT>::parser(const vector<pair<string, vector<vector<string>>>>& g,
	...
	for (auto x : g)
		for (auto y : x.second) {
			cout <<'\t'<< to_stdstr(x.first) <<'=';
			for (auto z : y) cout << to_stdstr(z);
			cout << endl;
		}
	// cout << "G:\n";
	// for (auto x : G) {
...
void parser<CharT>::scan(const item& i, size_t n, CharT ch) {
	...
	//DBG(print(cout, i) << ' ';)
	//DBG(print(cout << "scanned " << ch << " and added ", j) << "\n";)
	/*
	stringstream ss;
...
bool parser<CharT>::recognize(const typename parser<CharT>::string s) {
	//DBG(bool print_optimizations = o::enabled("parser-benchmarks");)
	//DBG(cout << "recognizing: " << to_stdstr(s) << endl;)
...
bool parser<CharT>::to_tml_facts(ostream_t& ss, T &&pt) const {
	auto str_rel_output = [&ss, this] (string rel, size_t id,
		vector<variant<size_t, typename parser<CharT>::string>> args)
	{
		ss << to_stdstr(rel) << "(" << id;
		for (auto &a : args) {
...
#ifdef DEBUG
			if (std::holds_alternative<size_t>(a))
				cout << " " << std::get<size_t>(a);
			else cout << " " << to_stdstr(std::get<string>(a));
#endif
		}
		ss << ").\n";
	};
...
template <typename cb_enter_t, typename cb_exit_t, typename cb_revisit_t,
	typename cb_ambig_t>
bool parser<CharT>::traverse_forest(const nidx_t &root, cb_enter_t cb_enter,
...
template <typename cb_enter_t, typename cb_exit_t, typename cb_revisit_t,
	typename cb_ambig_t>
bool parser<CharT>::_traverse_forest(const nidx_t &root, std::set<nidx_t> &done,
...
template <typename CharT>
uintmax_t parser<CharT>::count_parsed_trees() {
	nidx_t root(start, { 0, inputstr.length() });
...
template <typename CharT>
vector<typename parser<CharT>::arg_t> parser<CharT>::get_parse_graph_facts() {
	vector<arg_t> rts;
...
template <typename CharT>
std::string parser<CharT>::to_tml_rule(const nidx_t nd) const {
	std::stringstream ss;
...
template <typename CharT>
std::string parser<CharT>::to_stdstr(const string& s) const {
	return to_string(to_string_t(s));
}

template <typename CharT>
std::string parser<CharT>::to_stdstr(const char32_t& ch) const {
...
bool parser<CharT>::forest() {
	...
	emeasure_time_end(tspfo, tepfo) << " :: preprocess time ," <<
						"size : "<< count << "\n";
	cout <<"sort sizes : " << sorted_citem.size() << " " <<
						rsorted_citem.size() <<" \n";
	// build forest
	...
	cout <<"# parse trees " << count_parsed_trees() <<endl;
...
template <typename CharT>
bool parser<CharT>::build_forest ( const nidx_t &root ) {
...
template <typename CharT>
bool parser<CharT>::build_forest2 (const nidx_t &root) {
	if (!root.nt()) return false;
	if (pfgraph.find(root) != pfgraph.end()) return false;
...
template <typename CharT>
vector<typename parser<CharT>::node_children> parser<CharT>::get_children(
	const nidx_t nd, bool all) const
{
	...
	bool amb = false;
	if (print_ambiguity && it->second.size() > 1) amb = true, cout <<"'"
...
template <typename CharT>
typename parser<CharT>::string parser<CharT>::shorten(const string& s,
...
template <typename CharT>
typename parser<CharT>::string parser<CharT>::flatten(const nidx_t p) const {
	stringstream ss;
...
template <typename CharT>
void parser<CharT>::down(const nidx_t& p, const actions& a) const {
...
template <>
std::basic_string<char> parser<char>::epsilon()         const { return  "ε"; }
template <>
std::basic_string<char32_t> parser<char32_t>::epsilon() const { return U"ε"; }
...
template class idni::parser<char>;
template class idni::parser<char32_t>;

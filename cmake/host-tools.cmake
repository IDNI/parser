...
# native build is driven on the side through ExternalProject_Add purely to
# get a runnable tgf out of it. Either way this module sets TAU_TGF_EXECUTABLE
# (the binary to run) and TAU_TGF_DEPEND (what to depend on to have it built).

# Source tree the side native build configures. Defaults to this project's
# own sources; a project embedding this one (e.g. as a submodule) overrides
# it, before including this module, to that submodule's path.
set(TAU_PARSER_HOST_SOURCE_DIR "${PROJECT_SOURCE_DIR}"
	CACHE PATH "Source tree used for the native host build of tgf")
...
if(NOT CMAKE_CROSSCOMPILING)
	...
	# Configured fresh and without TAU_PARSER_BUILD_EMSCRIPTEN, so
	# TAU_PARSER_HOST_SOURCE_DIR's CMakeLists.txt never calls
	# set_emscripten_toolchain() and CMake picks up the build host's own
	# compiler. tgf only: no tests, no examples, no FTXUI (its REPL is
	# unused here, and skipping it avoids fetching it from source).
	...
	ExternalProject_Add(${TAU_TGF_DEPEND}
		SOURCE_DIR "${TAU_PARSER_HOST_SOURCE_DIR}"
		...
		# revisits it, so moving the source would leave the old generator

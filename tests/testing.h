...
namespace idni::testing
{
				   ...
				   test_options opts = {})
	{
		...
		{
			ss << "\n\n\t# parsing input " << quoted(to_std_string(input)) << " [" << input.size() << "]\n\n";
			if (verbosity > 1)
				g.print_internal_grammar(
		...
		if (found && (opts.dump || opts.contains.size()))
		{
			auto cb_enter = [&opts, &contained, &ss](const auto &n)
			{
				if (opts.contains == n->first.to_std_string())
					contained = true;
				if (opts.dump)
					ss << "\t#\t entering: \"" << n->first.to_std_string() << "\"\n";
			};
			auto cb_exit = [&opts, &ss](const auto &n, const auto &)
			{
				if (opts.dump)
					ss << "\t#\t exiting: \"" << n->first.to_std_string() << "\"\n";
			};
		...
		{
			if (opts.contains == parser<T>::tree::get(n).value.first.to_std_string())
		...
		if (verbosity > 0)
		{
			...
			if (ambiguity)
				ss << " because of ambiguity";
			if (expect_fail)
				ss << " but expected FAIL => " << (found ? "FAIL" : "SUCCESS") << "\n\n",
					ss << "\t# error_expected \"" << opts.error_expected << "\"\n";
			ss << "\n";
		}
#ifndef PARSER_BINTREE_FOREST
		if (found)
		{
			if (verbosity > 0)
				...
				ss << "\t# is_ambiguous " << (r.is_ambiguous() ? "yes" : "no") << "\n",
					ss << "\t# count_trees "
					   << f->count_trees() << "\n";
		}
#endif
		if (found && verbosity > 1)
		{
			...
			auto cb_next_g = [&r, &print, &ss](parser<T>::pgraph &g)
			{
				...
				if (print.rules)
					to_tml_rules<T, T, typename parser<T>::pgraph>(ss << "\n\t# TML rules:\n\n", g),
						ss << "\n";
				return true;
			};
			...
			if (print.facts)
				to_tml_facts<T, T>(ss << "\n\t# TML facts:\n\n",
					...
					ss << "\n";
#endif
			ss << "\t# terminals parsed: `" << to_string(r.get_terminals()) << "`\n";
			if (verbosity > 2)
				test_out<T>(c, g,
		...
		if (ambiguity && verbosity > 0)
			r.print_ambiguous_nodes(ss);
#endif
		if (expect_fail)
			found = msg.find(opts.error_expected) ==
	...
	{
		test_is_neg = opts.error_expected == "";
		if (!check())
			return info(std::cout), true;
		auto str2ntids = [&nts](const std::set<std::string> &list)
		{
	...
	void process_args(int argc, char **argv)
	{
		...
		for (auto it = args.begin(); it != args.end(); ++it)
		{
			...
			else if (opt == "-enable_autodisambg")
				auto_disambg = true;
			else if (opt == "-disable_autodisambg")
				auto_disambg = false;
...
#endif // __IDNI__PARSER__TESTING_H__

...
struct tml_parser {
	...
	std::unique_ptr<typename parser<char32_t>::pforest> parse(
		const std::basic_string<char32_t>& s)
	{
...
private:
	...
	char_class_fns<char32_t> cc;
	grammar<char32_t> g;
	parser<char32_t> p;
	prods<char32_t> t(size_t tid) { return prods<char32_t>(ts[tid]); }
	prods<char32_t> nt(size_t ntid) { return prods<char32_t>(
						lit<char32_t>(ntid, &nts)); }
	nonterminals<char32_t> load_nonterminals() const {
		nonterminals<char32_t> nts{};
		for (const auto& nt : {
	...
	char_class_fns<char32_t> load_cc() {
		return predefined_char_classes<char32_t>({
			U"eof",
			U"space",
			U"digit",
			U"xdigit",
